...
\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{assets/classification_resnet18_baseline_analysis/test_confusion_matrix.png}
\caption{Confusion matrix for the pretrained ResNet-18 baseline. The strongest off-diagonal blocks correspond to visually similar breeds rather than random category failures.}
\label{fig:classification_confusion}
\end{figure*}

\begin{figure}[t]
\centering
\includegraphics[width=0.88\linewidth]{assets/classification_resnet18_baseline_analysis/test_mistakes.png}
\caption{Representative classification mistakes from the pretrained ResNet-18 baseline. The tall mistake grid is shown as a separate figure instead of being forced beside the confusion matrix, because their aspect ratios are very different.}
...
\begin{figure*}[t]
\centering
\includegraphics[height=0.86\textheight]{assets/segmentation_60_loss_mask_grid.png}
\caption{Qualitative mask examples for the required segmentation losses. The visual comparison complements Table~\ref{tab:segmentation}: Dice improves overlap, but all losses still struggle around thin structures and boundaries.}
...
\caption{Segmentation failure cases. The most persistent errors occur at object boundaries, thin structures, and low-contrast regions rather than in the coarse pet/background separation.}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{assets/detection_yolov8n_results.png}
\caption{YOLOv8n training summary on VisDrone. The lightweight baseline improves steadily but remains limited by low recall and small-object localization difficulty at 640-pixel input size.}
\label{fig:detection_v8n_curves}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{assets/detection_yolov8s_results.png}
\caption{YOLOv8s training summary on VisDrone. Compared with YOLOv8n, the larger model improves precision, recall, mAP50, and mAP50-95, confirming that capacity helps in dense aerial scenes.}
\label{fig:detection_v8s_curves}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.49\textwidth]{assets/detection_yolo11m_img960_metrics.png}
\includegraphics[width=0.49\textwidth]{assets/detection_yolo11m_img960_losses.png}
...
\caption{A later tracking snapshot with accumulated line count. The figure also shows remaining limitations: some visible road users are difficult to detect or track under rain, distance, and occlusion.}
